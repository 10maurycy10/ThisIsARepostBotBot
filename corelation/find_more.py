...
dbc = db.cursor()
dbc.execute("select parent, id, hasbeenuserscraped,username from known_comments;")
...
for (pid,cid,hasbeenscraped,uname) in parent:
    ...
    try:
        ...
        if c.author != None:
            ...
            if (not (newbot in bots)) and (not (newbot in nonbots)):
                print(BOLD + GREEN + "NEW BOT CANDIDATE " + uname + " -> '" + c.author.name + "'" + RESET)
